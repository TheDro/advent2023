...
function p.getTree(frame)
	...
	local output = [[
	<div style="width: 1000px; height: 0px; background-color: midnightblue; position: relative; transform: translateY(-530px)">
		%s
	</div>
	]]
    ...
    for iTech = 1, #tier.research do
        ...
        local template = [[
            <div style="font-size: 0.7rem; line-height: 0.7rem; height: %spx; width: %spx; background-color: green; position: absolute; top: %spx; left: %spx">%s</div>
        ]]
